...
from tensorlayer.layers import ConcatLayer, ElementwiseLayer
...
def concat(layers, concat_dim=-1, name='concat'):
    """
    直接用 ConcatLayer 保留 prev_layer 链，
    输入必须是 Layer 对象列表。
    """
...
def merge(layers, name='merge'):
    """
    直接对 Layer 对象做 Elementwise 加法合并，
    确保输入都是 Layer 实例。
    """
    return ElementwiseLayer(layers, combine_fn=tf.add, name=name)
